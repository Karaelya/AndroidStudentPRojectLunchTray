...
class OrderViewModel : ViewModel() {
    ...
    private fun updateSubtotal(itemPrice: Double) {
        // Log to show incoming value
        Log.v("Show subtotal value", itemPrice.toString())
        if (_subtotal.value != null) {
            _subtotal.value = _subtotal.value?.plus(itemPrice)
        } else {
            _subtotal.value = itemPrice
        }
        // Log to show new subtotal value
        Log.v("Subtotal Value", _subtotal.value.toString())
        Log.v("Initial Subtotal", subtotal.value.toString())
